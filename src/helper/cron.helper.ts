...
export async function newsletterScheduledUploader(): Promise<void> {
  ...
  cron.schedule("*/3 * * * *", async () => {
    try {
      const findNewsletterMember: NewsletterMember[] =
        await prisma.newsletterMember.findMany({
          where: {
            status: "SUBSCRIBE",
          },
        });
      ...
      const findScheduledNewsletter: Newsletter[] =
      ...
      // read html file
      const html: string = await fs.readFile(
        "assets/html/newsletter.html",
        "utf8"
      );
